$(document).ready(function(){
	...
	*/
	(function(event){
		...
		var btnSVG = $('#addBtn svg'),
 		...
 		input.bind('input propertychange',function(event){
 			...
 			//输入则控件颜色变化
 			if(vinput !== ''){
 				okAddBtn.css('background-color',appSkin().mainColor);
	...
	$('#task-list').on('click','.clear',function(){
		model.clearTask('task',$(this).parent().find('i').text());
	});
	...
	function Model(){
		var addTask = function(s){
			var taskItem = [localStorage['yoone_task']];
			taskItem.push(s);
			localStorage.setItem('yoone_task',taskItem);
			render.init();
		};
	...
	//渲染器
	function Render(){
		var tasks,taskTitle,taskItemHTML='';
	...
	*/
	...
	$('#menu-btn').click(function(e){
		if($('.menu-content').css('left') == '-260px'){
			$('.menu-content').css('left','0px');
			$('.cover').fadeIn();
		};
		if($('.menu-content').css('left') == '0px'){
			hideSidebar();
			$('.cover').fadeOut();
		};
	});
	...
	$('.menu-item').click(function(e){
		...
		switch(data){
			case 'task':
				...
				break;
			case 'completed':
				MenuItemFunc.showList('completed');
				break;
			case 'skin':
				MenuItemFunc.changeSkin();
				break;
			case 'about':
				MenuItemFunc.about();
				break;
